import { NavRoot, NavBar, NavMenu, Image, Container, Paragraph, Button, NavToggle, NavOverlay, GridContainer, GridItem, Link } from 'mattkleyn-component-library';
        ...
        <NavRoot position='fixed' offsetTop='0rem' className='nav_root'>
            <NavBar direction='row' dock={`${isPastThreshold ? "left" : "none"}`} padding='md' maxWidth='md' className='nav_bar'>
                ...
                <NavToggle onClick={toggleMenu} className={`${isPastThreshold ? "nav_toggle--visible" : "nav_toggle--hidden"}`}>
                    {isMenuOpen ?                     
                        (<Paragraph level='md' className='nav_close_toggle'>Close</Paragraph>) :
                        (<Paragraph level='md'>Open</Paragraph>)
                    }
                </NavToggle>
                {isMenuOpen && (
